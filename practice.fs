...
let rec smallest = function 
    | ((head:int)::(head1::tail)) when tail = [] && head1 <= head-> head1
    | (head::(head1::tail)) when tail = [] && head1 >= head-> head
...
let rec sort_helper = function
    ...
    | (head::tail, res) -> sort_helper(delete(smallest(head::tail), head::tail), res @ smallest(head::tail)::[])
