...
let (.-.) x y = 
    ...
    let coupers_converted_x = convert_coupers x
    let silvers_converted_x = convert_silvers coupers_converted_x

    
    let coupers_converted_y = convert_coupers y
    let silvers_converted_y = convert_silvers coupers_converted_y

    
    let ((_,_,c1):int*int*int) = silvers_converted_y
    let ((_,s1,_):int*int*int) = silvers_converted_y 
    let ((g1,_,_):int*int*int) = silvers_converted_y
    let ((_,_,c2):int*int*int) = silvers_converted_x
    let ((_,s2,_):int*int*int) = silvers_converted_x 
    let ((g2,_,_):int*int*int) = silvers_converted_x
